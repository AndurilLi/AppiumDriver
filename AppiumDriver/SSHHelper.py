...
class SSHHelper:
    ...
    def exe_cmd(self, cmd, combine_error=False, nbytes=65536):
...
#     ssh = SSHHelper("10.197.60.109", "mxu", "mstr123","Output")
    ssh = SSHHelper("localhost")
    ssh.connect()
#     session = ssh.start_blocking_session("appium --log-no-colors", "appium.log")
#     time.sleep(5)
#     print "abcde"
#     ssh.close_blocking_session(session)
#     time.sleep(2)
    output = ssh.exe_cmd("lsd", False)
    print output.stdout
    print output.stderr
    print output.code
    ssh.disconnect()